...
my $summary = "All_MaleX_Individuals_GC_SUMMARY.txt";
...
foreach my $indiv(@SNPS){
	...
	my $name = "00"."$1";
	next unless ($name =~ /Xchr/)&&($name =~ /\dM\d/);
...
sub transitionScan{
	...
	foreach my $line(@chunk){
		...
		#don't count indels, just single uppercase letters
		...
		my @alleles = split /\//, $gt;
		#print "pfalleles=$pf,pmAlleles=$pm\t\talt=$alt,ref=$ref\tgt=$gt, alleles - @alleles\n";
	...
	if (($newchrom == 0)&&($hetcount == 0)&&(scalar @chunk == scalar @hethom)){
		...
		if (("$hethom[0]" eq "$hethom[-1]") && ("$hethom[0]" eq "$hethom[1]") && ("$hethom[1]" eq "$hethom[2]")){
